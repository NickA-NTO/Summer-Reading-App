...
const JWKS_TTL_MS = 60 * 60 * 1000; // 1 hr
const GOOGLE_ISSUERS = new Set([
  "https://accounts.google.com",
  "accounts.google.com",
]);
// Per-JWKS-URL cache (keyed by url) so Google + TimeBack/Cognito don't clobber
// each other's keys.
const _jwksCacheByUrl = new Map(); // url -> { keys, fetchedAt }

async function fetchJwks(url, forceRefresh = false) {
  const cached = _jwksCacheByUrl.get(url);
  if (!forceRefresh && cached && Date.now() - cached.fetchedAt < JWKS_TTL_MS) {
    return cached.keys;
  }
  // #19 audit follow-up: bound the JWKS fetch so a slow IdP response can't
  // stall the whole login function until Vercel's gateway kills it. 3 s is
  // generous — typical JWKS reply is sub-100 ms.
  ...
  // #19: don't cache an empty array. If an IdP ever returns a 200 with no
  // keys, pinning [] for an hour would 100% fail every login. On empty:
  // throw so the caller's retry path runs, AND don't mutate the cache.
  if (keys.length === 0) {
    throw new Error("jwks_empty_response");
  }
  _jwksCacheByUrl.set(url, { keys, fetchedAt: Date.now() });
  return keys;
}

/**
 * Generic OIDC id_token verifier (RS256 + JWKS). Used for BOTH Google login
 * and "Sign in with TimeBack" (Cognito). Verifies signature against the IdP's
 * JWKS, then iss / aud / exp / iat. Returns the decoded claims.
 *
 * @param {string} idToken
 * @param {Object} opts
 * @param {string}      opts.jwksUrl   — IdP JWKS endpoint
 * @param {Set<string>|string[]} opts.issuers — acceptable `iss` values
 * @param {string}      [opts.audience] — expected `aud` (the client_id)
 */
export async function verifyOidcIdToken(idToken, { jwksUrl, issuers, audience } = {}) {
  if (!idToken || typeof idToken !== "string") {
    throw new Error("invalid_id_token");
  }
  if (!jwksUrl) throw new Error("missing_jwks_url");
  const issuerSet = issuers instanceof Set ? issuers : new Set(issuers || []);
  ...
  if (issuerSet.size && !issuerSet.has(payload.iss)) throw new Error("invalid_iss");
...
// Google login id_token verifier — thin wrapper over the generic OIDC verifier.
...
// "Effective" admin = a true admin who has NOT switched on Student Mode.
// Student Mode (#studentmode) lets an operator experience the real student
// flow: it strips admin TESTING privileges (the 2-attempt-cap bypass, catalog
// & grade see-all, answer reveal, admin debug UI) by making every student-flow
// gate treat the operator as an ordinary kid. The toggle ITSELF stays gated on
// TRUE isAdmin (see api/admin set-student-mode) so the operator can always turn
// it back off. Time-holds are deliberately NOT stripped here — those are keyed
// off bypassQuizHolds so testing stays fast even in Student Mode.
export function isEffectiveAdmin(email, profile) {
  return isAdmin(email) && !(profile && profile.studentMode);
}
...
const HARDCODED_BYPASS_QUIZ_HOLDS = new Set([
  ...
  // Pilot tester — added so they can run the kid flow without the 15-min
  // wait. Remove after the test pass.
  "raditya.dwiprasta@trilogy.com",
  "raditya.dwiprasta@alpha.school",
]);
