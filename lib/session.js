...
//     (HMAC is one-way).
//   - Brute force: kid could submit each idx 0..3 in 4 separate
//     requests to find the correct one. Rate-limit + per-day attempt
//     cap mitigate. Future hardening: include a per-attempt nonce
//     server-side to make replay impossible.
//   - Schema/cache changes never affect grading — the token's HMAC
//     depends only on (secret, bookId, qText, correctIdx) which are
//     stable for the life of the question.
export async function signQuizAnswer(bookId, qText, correctIdx) {
  const secret = authSecret();
  const payload = `${bookId}|${qText}|${Number(correctIdx)}`;
  try {
    const key = await crypto.subtle.importKey(
      "raw", enc.encode(secret),
      { name: "HMAC", hash: "SHA-256" }, false, ["sign"]
...
export async function verifyQuizAnswer(bookId, qText, chosenIdx, token) {
