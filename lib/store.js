...
export async function setReadingSessionQuizOutcome({
  ...
  totalQuestions = null,   // it can emit the accuracy AssessmentEvent to TimeBack
  ts = Date.now(),
}) {
  const r = redis();
  if (!r) return { ok: false, reason: "no_redis" };
  ...
  const payload = {
    ...
    fraudStatus,
    fraudReason,
    // Raw quiz accuracy — read back at retell finalize to emit the accuracy
    // AssessmentEvent alongside the XP GradeEvent (TimeBack accuracy metric).
    correctQuestions,
    totalQuestions,
    retellOutcome: null,
    retellAttempt: null,
