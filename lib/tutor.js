...
//   ≥10 = p1 in XP table — strong retell with detail
//   7-9 = p2 in XP table — passed but thin
//    <7 = fF in XP table — title regurgitation, off-topic, etc.
//
// TUTOR_CLEAR_PASS_SCORE is a SEPARATE, lower gate: the score a kid must
// reach on their FIRST answer (with every axis non-zero) to finish WITHOUT
// a follow-up question. Set below the p1 cutoff so a strong single-turn
// retell can end immediately instead of always being re-prompted on camera.
...
export async function gradeConversation({ book, ageGrade, transcript, mode = "commit", bookSummary }) {
  ...
  const sys = [
    ...
    "from the body, the total MUST stay below 7 (fail) — reciting the",
    "cover is not retelling the book.",
    "",
    ...
    "must total below 7 (fail) — they didn't show they read it. But any genuine Bucket-B",
