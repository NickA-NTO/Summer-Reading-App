...
export async function previewGradeFirstTurn({ book, ageGrade, studentText, bookSummary }) {
  ...
  const sys = [
    ...
    "",
    "==== ANTI-GUESSING RULES — these matter most ====",
    "",
    "A student who didn't read the book can still produce GENERIC details by guessing from",
    "the title, cover, or genre. Detect this and score it as zero — only DETAILS THAT APPEAR",
    "IN THE BOOK REFERENCE ABOVE count toward the rubric.",
    "",
    "Generic = anything a kid could plausibly say without reading. Score these as 0:",
    "- 'It was about a castle / wizards / dragons' for any fantasy book (genre trope)",
    "- 'There was a forest / animals / a princess' for any fairy tale (genre trope)",
    "- 'They learned a lesson / were friends / lived happily' (generic kids' book ending)",
    "- 'It was about [title noun]' — e.g. 'a pig' for Charlotte's Web (cover/title)",
    "- 'There was a dog / a cat / a kid' (if not in the reference above)",
    "",
    "Specific = facts that match the BOOK REFERENCE and aren't predictable from cover/genre.",
    "Score these as 2-3:",
    "- Named characters: 'Wilbur', 'Templeton', 'Charlotte'",
    "- Named items or places: 'Mr. Zuckerman's barn', 'the slop bucket'",
    "- Specific events that match the reference: 'Charlotte wrote SOME PIG in her web'",
    "- Unique details: 'Wilbur was the runt and Fern saved him'",
    "",
    "Worked example — book: Charlotte's Web. Student says: 'It's about a pig and a spider on a farm.'",
    "  Genre check: pigs + farms are guessable from the cover. Spider is in the title.",
    "  retell_quality: 1 (one sentence, generic-ish but on-topic)",
    "  character_recall: 1 (mentions creature-types but no names — guessable)",
    "  event_recall: 0 (no events described)",
    "  stayed_on_topic: 2 (on-topic but thin)",
    "  → 4/12 total — does NOT pass.",
    "",
    "Worked example — same book, student says: 'Wilbur is a pig who would have been killed but",
    ...
    "  Example: 'The book is about red fish and blue fish' for 'One Fish Two Fish Red Fish Blue Fish' → 0/0 (title words don't count).",
    ...
    "- If a younger student (PK/K) gives one specific accurate detail, that's worth full credit on that axis even if the rest is generic — they may need follow-up prompting.",
    "",
...
export async function gradeConversation({ book, ageGrade, transcript, mode = "commit", bookSummary }) {
  ...
  const sys = [
    ...
    "",
    "A student who didn't read the book can still produce GENERIC details by guessing from",
    "the title, cover, or genre. Only DETAILS THAT APPEAR IN THE BOOK REFERENCE ABOVE count.",
    "",
    "Generic (score 0): castle/wizard/dragon for any fantasy; forest/animals/princess for any",
    "fairy tale; 'they were friends', 'they learned a lesson', 'lived happily ever after';",
    "'it was about [title noun]'; 'there was a dog/cat/kid' (when not in the reference).",
    "",
    "Specific (score 2-3): named characters from the reference; named items/places; events",
    "that match the reference; unique details a non-reader couldn't guess.",
    "",
    "If the kid only produces generic content across both their turns, the rubric total",
    "must come out below 7 (fail) — they didn't demonstrate that they read the book.",
    "",
    "==== OTHER STRICT RULES ====",
    ...
    "- Younger students (PK/K) get partial credit if at least one detail is genuinely specific to the book — they may have needed prompting to produce more.",
    "",
    ...verdictRules,
    "",
