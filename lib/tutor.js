...
export const TUTOR_CLEAR_PASS_SCORE = 6;
...
// Sum of the four rubric axes. Range 0..8. Helper so the api layer
// doesn't have to know the axis names.
export function totalRubricScore(grade) {
  if (!grade) return 0;
  return (
    clamp02(grade.retell_quality) +
    clamp02(grade.character_recall) +
    clamp02(grade.event_recall) +
    clamp02(grade.stayed_on_topic)
  );
}
...
// That ordering is deliberate — asking about a concrete EVENT tends to
// shake loose character names too ("when Ned got mad at the cow..."),
// whereas asking "who's in it?" alone often gets a one-name answer.
export function pickWeakAxis(grade) {
  if (!grade) return "event_recall";
  const axes = [
    ["event_recall",     clamp02(grade.event_recall)],
    ["character_recall", clamp02(grade.character_recall)],
    ["retell_quality",   clamp02(grade.retell_quality)],
    ["stayed_on_topic",  clamp02(grade.stayed_on_topic)],
  ];
...
export async function previewGradeFirstTurn({ book, ageGrade, studentText }) {
  ...
  const sys = [
    ...
    "",
    "This is their FIRST answer to \"Can you tell me what the book was about?\". Score each dimension 0-2 based on this one response only:",
    "- retell_quality: Did they describe the overall plot or what the book is about?",
    "- character_recall: Did they identify or describe at least one character?",
    "- event_recall: Did they mention a specific event, scene, or detail from the book?",
    "- stayed_on_topic: Did they stay focused on this book (not a different one, not unrelated chatter)?",
    "",
    "Be lenient with young students — partial answers and fragments are fine if they show real understanding of THIS book.",
    "Be strict about wrong-book responses — if it's about a different story, score zero on event/character.",
    "",
    'Reply with strict JSON only: {"retell_quality": 0-2, "character_recall": 0-2, "event_recall": 0-2, "stayed_on_topic": 0-2}',
  ].join("\n");
  ...
  // a kid who got 2/2/2/0 with nothing on topic-stay shouldn't skip the
...
export async function gradeConversation({ book, ageGrade, transcript, mode = "commit" }) {
  ...
  const verdictRules =
    mode === "commit"
      ? [
          ...
          `- true if total score ≥ ${TUTOR_PASS_SCORE}/8 AND the responses are clearly about this book`,
  ...
  const sys = [
    ...
    `Student age group: ${ageGrade}.`,
    "",
    `The student gave ${turnLabel}. Score each dimension 0-2 based on the FULL conversation:`,
    ...
    "",
    'Reply with strict JSON only: {"retell_quality": 0-2, "character_recall": 0-2, "event_recall": 0-2, "stayed_on_topic": 0-2, "overall_pass": true|false' + (mode === "soft" ? "|null" : "") + ', "feedback": "one short positive sentence for the student"}',
  ].join("\n");
