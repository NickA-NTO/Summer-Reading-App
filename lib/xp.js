...
// Standard quiz = 5 Q × 1 min = 5 min. Emergent = 3 Q × 1 min = 3 min.
// Flat regardless of attempt count — the second attempt is shorter
// (familiar questions) but we don't reward retries with extra time.
export const QUIZ_MIN_PER_QUESTION = 1;
export const QUIZ_QUESTIONS_FULL = 5;
export const QUIZ_QUESTIONS_EMERGENT = 3;
export const QUIZ_TIME_FULL_MIN     = QUIZ_QUESTIONS_FULL     * QUIZ_MIN_PER_QUESTION;  // 5
export const QUIZ_TIME_EMERGENT_MIN = QUIZ_QUESTIONS_EMERGENT * QUIZ_MIN_PER_QUESTION;  // 3
 ...
 *  OUTCOME_RATIOS. The p1/p2 distinction encodes quality tier
 *  instead of attempt count:
 *    ≥ 10 / 12 → "p1" — clear pass, strong retell (1.30× when quiz p1)
 *    7-9 / 12  → "p2" — marginal pass, thin retell (1.15× when quiz p1)
 *     < 7 / 12 → "fF" — fail, no retell bonus (1.00× when quiz p1)
 *
 *  This replaces outcomeCode(passed, attempt) for retell in
 *  api/tutor.js. A "marginal pass" retell (5-6/12) used to earn the
 *  same XP as a perfect 8/8 — same outcomeKey, same multiplier.
 *  Tiering by rubric total preserves the OUTCOME_RATIOS table while
 *  letting XP reflect actual retell quality.
 */
export function retellOutcomeFromRubric(rubricTotal) {
  const t = Number(rubricTotal);
  if (!Number.isFinite(t) || t < 7) return "fF";
  if (t < 10) return "p2";
  return "p1";
}
