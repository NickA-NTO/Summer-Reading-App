...
export function containsProfanity(text) {
  ...
  // Token-exact (NOT substring) match on the compacted forms. Substring matching
  // caused Scunthorpe false-positives — short slur fragments like "nig" matched
  // innocent words ("night", "tonight", "knight"), "rape"→"grape"/"therapist",
  // "cock"→"cockpit" — which broke the spoken retell: a kid saying "that night"
  // (a core line of The Very Hungry Caterpillar) tripped the child-safety filter
  // → endless gentle-redirect loop (#QC). The spaced-letter join in _toCompact
  // still collapses obfuscation like "n i g g e r" → one token, so real slurs are
  // caught as exact compact tokens; whole-word slurs are already caught above.
  const compacts = _candidateForms(text);
  for (const c of compacts) {
    for (const tok of c.split(/\s+/)) {
      if (tok && BLOCKED_COMPACT.has(tok)) return true;
    }
  }
  return false;
}
