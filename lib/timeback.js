...
// Keep a fire-and-forget promise alive past the HTTP response. On Vercel the
// function instance is frozen/killed the moment the handler returns, which
// truncates any un-awaited work mid-flight (the reason intermittent Caliper
// events silently vanished: the POST + token mint + retry-queue write never
// finished). waitUntil() registers the promise with the runtime so the
// instance stays alive until it settles. Outside a request context (local
// dev, tests, scripts) waitUntil throws — we swallow that and let the promise
// run on its own, which is fine when the process isn't being frozen.
function keepAlive(promise) {
  try {
    waitUntil(promise);
  } catch {
    /* no request context (local/test) — the promise still runs to completion
       because the process isn't being suspended. */
  }
}
...
// Events base URL (no envelope), honoring the same enable gate as
// postCaliperEnvelope. Returns null when emission is disabled.
export function eventsBaseUrl() {
  return endpointUrl();
}
...
/**
 * POST a raw event body (bare Caliper event, NOT the {sensor,data} envelope)
 * or a heartbeat payload directly to the events API. Used by the heartbeat
 * route for SessionEvent (LoggedIn) + /heartbeat, which the platform expects
 * un-enveloped. Mints + attaches the M2M Bearer. Gated by eventsEnabled().
 *
 * @param {object} body   JSON body to POST
 * @param {object} [opts] { path: "" | "/heartbeat" }
 */
export async function postEventRaw(body, opts = {}) {
  const base = endpointUrl();
  if (!base) return { ok: false, reason: "no_endpoint_configured" };
  let token;
  try {
  ...
  try {
    ...
    return { ok: false, reason: "http_error", status: r.status, body: text };
  } catch (err) {
...
export function sendCaliperEnvelopeAsync(envelope, ctx = {}) {
  ...
  // The events API accepts EXACTLY ONE event per envelope (a multi-event
  // data[] → 400 "Expected exactly one event"). Builders may return an
  // envelope with several events (e.g. per-question accuracy); split it into
  // one single-event envelope per event so each posts (and retries)
  // independently. (#one-event-per-envelope)
  const data = Array.isArray(envelope.data) ? envelope.data : [];
  const singles = data.length > 1
    ? data.map((ev) => ({ ...envelope, data: [ev] }))
    : [envelope];
  for (const single of singles) {
    // Don't block the response on TimeBack — but DO keep the instance alive
    // until the POST (and any retry-queue write) settles via keepAlive/waitUntil.
    // Without that, Vercel freezes the instance on handler return and the
    // un-awaited work is silently truncated. (#emit-race)
    const work = postCaliperEnvelope(single)
      .then(async (result) => {
        if (!result.ok && result.reason !== "no_endpoint_configured") {
          await queueCaliperRetry(single, result);
        } else if (result.ok) {
