...
// Debug capture (temporary demo aid — remove post-demo). Records the last
// few BUILT envelopes + their send result so an admin can inspect exactly
// what Reading Spine emitted, even on success (where the envelope is
// otherwise dropped) or on preview (built-but-not-sent). Capped + 15-min
// TTL; carries the same PII the envelope already sends to TimeBack. Read via
// GET /api/admin?action=caliper-debug (admin-gated).
const DEBUG_KEY = "caliper:debug";
const DEBUG_CAP = 10;
const DEBUG_TTL_SEC = 900;
async function captureCaliperDebug(envelope, result) {
  // #14 — disabled by default. This stored child PII (the same envelope sent
  // to TimeBack) in Redis as a demo aid; production never needs to accumulate
  // it. Opt-in only via CALIPER_DEBUG=1 for a one-off debugging session.
  if (process.env.CALIPER_DEBUG !== "1") return;
  const r = redis();
  if (!r) return;
  try {
    await r.lpush(DEBUG_KEY, JSON.stringify({
      ts: Date.now(),
      transmitted: result?.ok === true,
      result,
      envelope,
    }));
    await r.ltrim(DEBUG_KEY, 0, DEBUG_CAP - 1);
    await r.expire(DEBUG_KEY, DEBUG_TTL_SEC);
  } catch { /* debug only — never break the caller */ }
}
export async function getCaliperDebugCapture({ max = DEBUG_CAP } = {}) {
  const r = redis();
  if (!r) return { ok: false, reason: "no_redis" };
  try {
    const raw = await r.lrange(DEBUG_KEY, 0, max - 1);
    const entries = (raw || [])
...
export function sendCaliperEnvelopeAsync(envelope) {
  ...
  postCaliperEnvelope(envelope)
    .then(async (result) => {
      await captureCaliperDebug(envelope, result); // temporary demo aid
