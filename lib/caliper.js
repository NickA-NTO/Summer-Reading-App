...
// ─────────────────────────────────────────────────────────────────
// Retell events — Caliper coverage for the oral retell assessment.
// ─────────────────────────────────────────────────────────────────
// The retell is a SEPARATE Assessment from the quiz so TimeBack can
// distinguish the two. Same identity + envelope pattern as quiz
// events, but:
//   - object.id uses /retell/<bookId>/ instead of /quiz/
//   - maxScore is 12 (4 rubric axes × 0-3 each)
//   - extensions carry the per-axis rubric scores + quality tier
//     (clear_pass / marginal / fail) + the p1/p2/fF outcome code
//     so TimeBack can apply its own tier-based XP rules if it wants
//     (matching our internal OUTCOME_RATIOS table).

const RETELL_MAX_SCORE = 12;

function retellAssessmentObject({ bookId, bookTitle }) {
  return {
    id: `${edAppId()}/retell/${encodeURIComponent(bookId)}`,
    type: "Assessment",
    name: `${bookTitle} — oral retell`,
    maxScore: RETELL_MAX_SCORE,
  };
}
function retellAttemptObject({ bookId, bookTitle, attemptNum = 1 }) {
  return {
    id: `${edAppId()}/retell/${encodeURIComponent(bookId)}/attempt/${attemptNum}`,
    type: "AttemptItem",
    isPartOf: retellAssessmentObject({ bookId, bookTitle }),
  };
}

/**
 * AssessmentEvent for the retell completion. Fires once per finalized
 * retell session regardless of pass/fail. Idempotent via uuidv5 over
 * (email, bookId, attemptNum).
 */
export function buildRetellAssessmentEvent(p) {
  const eventTime = new Date(p.eventTimeMs || Date.now()).toISOString();
  const attemptNum = p.attemptNum || 1;
  const eventKey = `retell_assessment|${p.email}|${p.bookId}|${attemptNum}`;
  return {
    "@context": CALIPER_CONTEXT,
    id: `urn:uuid:${uuidv5(eventKey)}`,
    type: "AssessmentEvent",
...
/**
 * GradeEvent for the retell rubric score (0-12 total).
 *
 * @param {Object} p
 * @param {string} p.email
 ...
 * @param {number} p.rubricTotal       — 0..12 sum of the four axes
 * @param {Object} [p.rubric]          — { retell_quality, character_recall,
 *                                         event_recall, stayed_on_topic } each 0-3
 * @param {string} [p.tier]            — "clear_pass" | "marginal" | "fail"
 * @param {string} [p.retellOutcome]   — "p1" | "p2" | "fF" (matches OUTCOME_RATIOS)
 * @param {string} [p.outcomeKey]      — composite "quizOutcome_retellOutcome"
 * @param {string} [p.bookGradeLevel]
 * @param {string} [p.studentGrade]
 * @param {number} [p.xpAwarded]       — internal XP for the full session
 * @param {boolean}[p.held]            — true when retell verdict was null (admin review)
 * @param {number} [p.eventTimeMs]
 */
export function buildRetellGradeEvent(p) {
  const eventTime = new Date(p.eventTimeMs || Date.now()).toISOString();
  const attemptNum = p.attemptNum || 1;
  const eventKey = `retell_grade|${p.email}|${p.bookId}|${attemptNum}`;
  const scoreKey = `retell_score|${p.email}|${p.bookId}|${attemptNum}`;
  const score = {
    id: `urn:uuid:${uuidv5(scoreKey)}`,
    type: "Score",
    scoreGiven: Number(p.rubricTotal) || 0,
    maxScore: RETELL_MAX_SCORE,
    scoredBy: {
      id: edAppId(),
      type: "SoftwareApplication",
  ...
  // Extensions — rubric breakdown, tier, outcome code, and the
  // session metadata TimeBack might want to credit/apply XP rules.
  // Standard Caliper "extensions" key — TimeBack consumers ignore
  // it if they don't recognize fields.
  const extensions = {};
  if (p.rubric) {
    extensions.rubric = {
...
/**
 * Matched-pair envelope (AssessmentEvent + GradeEvent) for one retell
 * completion. Mirrors buildQuizEventEnvelope's shape so TimeBack's
 * sensor ingestion treats them identically.
 */
export function buildRetellEventEnvelope(p) {
  ...
  return {
    ...
    sendTime: new Date().toISOString(),
    dataVersion: CALIPER_CONTEXT,
    data: [buildRetellAssessmentEvent(p), buildRetellGradeEvent(p)],
  };
}
