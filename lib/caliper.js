...
// Deterministic lesson sourcedId from a bookId. MUST match the scheme used by
// scripts/publish-timeback-catalog (sha1("asr-lesson|"+bookId), first 16 bytes,
// formatted as a v5 UUID) so every event's lesson reference resolves to a real
// published lesson. NOTE: the PM doc says to pass the lesson via
// `extensions.lesson.id`, but the live events API REJECTS a `lesson` key in
// extensions (400 unrecognized_keys — verified). The working placement is
// `object.assignable.id` = this lesson URL; XP then credits at lesson grain.
...
function buildGradeEvent(p) {
  ...
  return {
    ...
    object: {
      ...
      // assignable = the published LESSON for this book. This is how TimeBack
      // ties the event to the lesson (the doc's conceptual "lesson.id"); the
      // events API rejects a `lesson` key in extensions, so it rides here.
      // Verified live: XP credits under the lesson.
      assignable: {
