...
// Reading Spine publishes ONE lesson PER BOOK, all under a single unit (the
// "Lalilo model"): the course still appears as one link on TimeBack (one unit),
// but each book is its own lesson so completing a book counts toward mastery
// ("Mastered Units" on the dashboard actually counts completed LESSONS, not
// catalog units — verified against Lalilo, which has 1 unit + many lessons).
// Each event pins its own book-lesson so mastery attributes per book. The
// lesson sourcedId is the deterministic v5 of "asr-lesson|"+bookId — MUST match
// the published catalog (scripts build all 55 with the same scheme).
//
...
function lessonSourcedIdForBook(bookId) {
  const h = createHash("sha1").update(`asr-lesson|${bookId}`).digest();
...
function buildGradeEvent(p) {
  ...
  return {
    ...
    object: {
      ...
      // assignable = THIS BOOK's published lesson. This is how TimeBack ties
      // the event to the lesson (the doc's conceptual "lesson.id") and how a
      // completed book registers as a mastered lesson on the dashboard; the
      // events API rejects a `lesson` key in extensions, so it rides here.
      assignable: {
        id: lessonUrl(p.bookId),
...
export function buildQuizAccuracyEnvelope(p) {
  ...
  const endMs = p.eventTimeMs || Date.now();
  const nonce = p.eventNonce != null ? String(p.eventNonce)
    : `${p.attemptNum != null ? p.attemptNum : 1}|${new Date(endMs).toISOString().slice(0, 10)}`;
  const lesson = lessonUrl(p.bookId);
  const events = flags.map((ok, i) => {
    const qKey = `qresult|${p.bookId}|${sourcedId}|${nonce}|q${i + 1}`;
    const evId = uuidv5(qKey);
    return {
      "@context": CALIPER_CONTEXT,
      id: `urn:uuid:${evId}`,
      type: "GradeEvent",
      ...
      edApp: softwareApp(),
      object: {
        id: `${edAppId()}/attempt/quiz/${encodeURIComponent(p.bookId)}/${sourcedId}/${nonce}/q${i + 1}`,
        type: "Attempt",
        count: 1,
        assignee: { id: userUrl(sourcedId), type: "Person" },
        // The question is a resource that is part of this book's lesson, so the
        // per-question results roll up under the right lesson.
        assignable: {
          id: `${edAppId()}/quiz/${encodeURIComponent(p.bookId)}/q${i + 1}`,
          type: "DigitalResource",
          mediaType: "application/json",
          isPartOf: { id: lesson, type: "DigitalResourceCollection" },
        },
        extensions: { activityName: `${p.bookTitle || p.bookId} — question ${i + 1}` },
      },
      generated: {
