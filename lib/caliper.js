...
function buildGradeEvent(p) {
  ...
  // Our internal flags (fraudFlag/held/retake/studentGrade) live in our own
  // Redis records, NOT on the Caliper envelope.
  const extensions = { subject: "Reading", course: { id: courseUrl(courseSourcedId(p)) } };
  ...
  return {
    ...
    object: {
      ...
      assignable: {
        id: `${edAppId()}/${kind}/${encodeURIComponent(p.bookId)}`,
...
function buildAssessmentEvent(p) {
  ...
  return {
    ...
    extensions: { subject: "Reading", course: { id: courseUrl(courseSourcedId(p)) } },
  };
}
