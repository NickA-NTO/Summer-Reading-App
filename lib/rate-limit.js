...
export const LIMITS = {
  ...
  // #20 — the default authenticated GET on /api/auth/me runs loadProfile
  // + evaluateAchievementsForUser + several Redis reads on every call.
  // A logged-in client loop (or a buggy poller) could hammer Redis
  // unbounded. 120/min is very generous for legitimate use (page loads,
  // post-quiz refreshes, occasional polling) while capping a runaway
  // loop. Fails open like every other bucket.
  me:          { max: 120, windowSec: 60 },
};
