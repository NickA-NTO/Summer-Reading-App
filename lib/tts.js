...
// Steer tone for younger readers — lifted from the openai-tts "instructions"
// pattern. Applied to every synth so the catalog voice feels consistent.
// Tune here once instead of forking strings per call site.
const VOICE_INSTRUCTIONS =
  "Speak slowly, warmly, and clearly — like reading to a young child. " +
  "Use a friendly, encouraging tone. Pause briefly between sentences.";
...
// SHA-256 of "v3|" + voice + "|" + text → stable cache key. The "v3" prefix
// is a schema version we bump on any output-affecting change (voice catalog,
// model, sample rate, instructions string).
...
export async function synthAndStore(text, voice) {
  ...
  // gpt-4o-mini-tts is the current cost-efficient TTS model (also supports
  // the `instructions` field for tone steering). Older `tts-1` / `tts-1-hd`
  // are still available but lack instructions and cost the same / more.
