...
export const VOICES = {
  ...
  // #T28 — 'coral' retired (didn't sound good). A stored coral pref maps to
  // DEFAULT_VOICE via normalizeVoiceId + the server's unknown-voice coercion.
};
...
// 0.95 — 0.85 time-stretched tts-1 too aggressively: it garbled SHORT
// fragments (a bare option "A red bow tie" slurred to "Buh Hai") and made the
// male voices (Ash/Fable) sound echoey/robotic. 0.95 keeps a gentle slowdown
// without the artifacts; the K-2 "paused" feel comes from the wall-clock gap
// between question and choices in speakQuizQuestion (1200ms) plus speaking
// options inside a carrier sentence. (#T29/#T33)
...
// SHA-256 of "v6|" + voice + "|" + text → stable cache key. Bumped v5→v6 to
// discard every clip synthesized at the garbled 0.85 speed (#T29/#T33) so kids
// stop hearing the cached "Buh Hai". Bump on any output-affecting change: voice
// catalog, model, sample rate, speed, instructions.
