...
// Polly neural voices we expose. Ivy/Justin/Kevin are *child* voices —
// uniquely Polly, perfect for K-2.
export const VOICES = {
  Ivy:     { gender: "female", age: "child", label: "Ivy (child, female)" },
  Justin:  { gender: "male",   age: "child", label: "Justin (child, male)" },
  Kevin:   { gender: "male",   age: "child", label: "Kevin (child, male)" },
  Joanna:  { gender: "female", age: "adult", label: "Joanna (adult, female)" },
  Matthew: { gender: "male",   age: "adult", label: "Matthew (adult, male)" },
};
...
// SHA-256 of voice + "|" + text → stable cache key. Uses Web Crypto so it
// works on both Edge and Node runtimes.
export async function cacheKey(voice, text) {
  const enc = new TextEncoder().encode(`${voice}|${text}`);
