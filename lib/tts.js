...
// Polly neural voices — best-in-class for each category. Documented why
// each is selected:
//
//   Female Child → Ivy
//     The ONLY female child voice in the Polly catalog. Neural-supported.
//
//   Male Child → Kevin
//     The only neural-capable male child voice. (Justin is the other
//     male child but supports STANDARD engine only — much more robotic.)
//
//   Female Adult → Joanna
//     The most established neural female. Warm, conversational tone.
//     Also supports newscaster + conversational styles if we want them.
//     Beats Kendra (more nasal) and Salli (less natural) for narration.
//
//   Male Adult → Matthew
//     Most established neural male. Clear, friendly. Also has newscaster
//     style. Beats Joey (slightly older sounding) for K-2 narration.
//
// All four use the "neural" engine. If we ever want even more lifelike
// audio (at ~2x cost), the "generative" engine is available for Joanna
// and Matthew. Not enabled by default to stay within the $5 budget.
export const VOICES = {
